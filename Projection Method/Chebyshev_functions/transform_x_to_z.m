function z = transform_x_to_z(x,m,p) % x,m,p : inputs, k : output
...
z = (1 + x .* sec(pi/(2*m))) * ((p.zmax - p.zmin)./2) + p.zmin;  % z(x): function that maps the nodes x (x in [-1,1]) to the domain of the gridpoints for productivity (z in [p.zmin,p.zmax])
