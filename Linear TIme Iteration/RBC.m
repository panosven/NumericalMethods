...
x = zeros(3, nresp);     % creates a 3x80 matrix of x's, that is, the policy
...
k_irf = x(2,2:end);                  % Gives the IRF for Capital
...
% Plotting
figure;
subplot(3,2,1); plot(x(1,2:end), 'b','Linewidth',2);    % plots the policy funtion of the 1st variable of matrix x (3x80)                                                       % (1st variable is consumption) for the specified horizon 
title('Consumption');
grid on;
...
subplot(3,2,5); plot(R_irf, 'b', 'Linewidth',2);
